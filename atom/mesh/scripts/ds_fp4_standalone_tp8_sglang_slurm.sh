#!/usr/bin/env bash
#SBATCH --job-name=ds-fp4-standalone-tp8-sglang
...
#SBATCH --output=/it-share/yajizhan/slurm_logs/ds_fp4_standalone_tp8_sglang-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_logs/ds_fp4_standalone_tp8_sglang-%j.err
#
# Self-contained single-node (non-disaggregated) benchmark for DeepSeek-R1 MXFP4
# on SGLang. Mirrors ds_fp4_1p_tp8_1d_tp8_sglang_slurm.sh structure but strips
# all PD/Mooncake plumbing — used to isolate model vs PD-framework regressions.
#
# Workload sweep: ISL:OSL pairs (default "8192:1,1:1024") × CONC list.
#
# Usage:
#   mkdir -p /it-share/yajizhan/slurm_logs
#   sbatch ds_fp4_standalone_tp8_sglang_slurm.sh
#
# Override defaults via env, e.g.:
#   sbatch --export=ALL,ISL_OSL_LIST="8192:1024" ds_fp4_standalone_tp8_sglang_slurm.sh

set -euo pipefail

# ======================== configuration ========================
MODEL_PATH="${MODEL_PATH:-/mnt/models/DeepSeek-R1-0528-MXFP4}"
DOCKER_IMAGE="${DOCKER_IMAGE:-rocm/atom-dev:mesh-sglang-latest}"
CONTAINER="${CONTAINER:-atom_sglang_standalone_${SLURM_JOB_ID}}"
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_logs/$(date +%m%d)_ds_fp4_standalone_tp8_sglang_${SLURM_JOB_ID}}"
...
MODEL    : ${MODEL_PATH}
IMAGE    : ${DOCKER_IMAGE}
BACKEND  : sglang (standalone, no PD)
...
lm_eval --model local-completions \
    --model_args "model=${MODEL_PATH},base_url=http://127.0.0.1:${PORT}/v1/completions,num_concurrent=${GSM8K_NUM_CONCURRENT},max_retries=1,tokenized_requests=False,trust_remote_code=True" \
    --tasks gsm8k \
...
json_files = sorted(result_dir.glob('standalone-sglang-*.json'))
if not json_files:
...
# Real inference probe — /v1/models returns 200 long before weights/CUDAGraph
# are ready, so we send an actual completion and require non-empty text.
...
echo "[server] launching standalone sglang on ${NODE}"
srun --nodelist="$NODE" --nodes=1 --ntasks=1 bash -lc "
