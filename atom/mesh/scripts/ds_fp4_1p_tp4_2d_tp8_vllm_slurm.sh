#!/usr/bin/env bash
#SBATCH --job-name=ds-fp4-vllm-1p-tp4-2d-tp8
...
#SBATCH --output=/it-share/yajizhan/slurm_logs/ds_fp4_vllm_1p_tp4_2d_tp8-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_logs/ds_fp4_vllm_1p_tp4_2d_tp8-%j.err
#
# Self-contained 1P+2D PD-disaggregated benchmark for DeepSeek-R1 MXFP4.
#   prefill: TP=4 (4 GPUs, other 4 idle), decode: TP=8 (2 instances), vLLM MooncakeConnector RDMA KV transfer.
...
#   sbatch ds_fp4_1p_tp4_2d_tp8_vllm_slurm.sh
#
# Override defaults via env:
#   sbatch --export=ALL,LOAD_DUMMY=,ISL_LIST="1024,8192" ds_fp4_1p_tp4_2d_tp8_vllm_slurm.sh
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_logs/$(date +%m%d)_ds_fp4_vllm_1p_tp4_2d_tp8_${SLURM_JOB_ID}}"
...
echo "[decode-1] launching vLLM kv_consumer on ${DECODE_NODE_1}"
srun --nodelist="$DECODE_NODE_1" --nodes=1 --ntasks=1 bash -lc "
    docker exec -d '${CONTAINER}' bash '${LOG_ROOT}/scripts/decode_1.sh'
"
echo "[decode-2] launching vLLM kv_consumer on ${DECODE_NODE_2}"
