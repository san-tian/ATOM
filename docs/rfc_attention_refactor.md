...
| `backend.py` | vLLM backend and metadata builder classes. `AiterMhaBackendForVllm` / `AiterMlaBackendForVllm` implement the vLLM backend contract (`get_name() = "CUSTOM"`, `get_kv_cache_shape`, `full_cls_name`, etc.). `AiterMhaMetadataBuilderForVllm` builds `AttentionMetaData` with `AiterFlashAttentionMetadataForPluginMode` for prefill/decode. `AiterMlaMetadataBuilderForVllm` inherits vLLM's `MLACommonMetadataBuilder` and adds persistent worker buffer management. |
| `metadata.py` | vLLM-specific metadata dataclasses: `AiterFlashAttentionMetadataForPluginMode` (MHA prefill/decode/chunk metadata), `AiterMLACommonMetadataForPluginMode` (MLA metadata with decode/prefill phases), `vllmDeepseekV32IndexerMetadata` (sparse indexer metadata). Also contains sparse metadata builder method-container mixins used by `mla_sparse.py`. |
