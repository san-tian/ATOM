...
First, the code was hard to understand. A class in `atom/model_ops` could behave differently depending on whether the current mode was native, vLLM, or SGLang. The real methods on a class were not always visible in the source file.

Second, vLLM forced strong interface constraints on ATOM attention. In the old design, ATOM-vLLM constructed vLLM standard `Attention` or `MLAAttention`. Because of that, ATOM code had to follow vLLM attention layer details, including backend selection, metadata builder shape, KV cache spec, output buffer behavior, and forward context behavior.

Third, the `atom/plugin` folder mixed many unrelated concerns. vLLM-only attention code, SGLang-only code, shared plugin code, and native attention helper code were placed together. This made future changes risky.

Fourth, native ATOM attention and ATOM-vLLM attention were coupled at multiple levels:
...
class Attention:
    def __new__(cls, *args, **kwargs):
        ...
        return AtomNativeAttention(*args, **kwargs)
```
...
`AttentionForVllmMHA` directly implements the MHA vLLM attention path. It inherits `nn.Module` and vLLM `AttentionLayerBase`. It does not inherit native `PagedAttentionImpl`.

`AttentionForVllmMLA` directly implements the dense MLA and sparse MLA vLLM forward path. It implements vLLM layer requirements and contains its own vLLM-specific forward logic. It still subclasses native `MLAAttention` for a limited reason: native `MLAAttention` owns the MLA module state and weight preparation, such as `q_proj`, `o_proj`, `kv_b_proj`, MLA dimensions, `topk_indices_buffer`, FP8/FP4 post-load processing, and related projection weights. The vLLM path reuses that state layout, but it does not reuse the native `forward()` control path.

This is an intentional asymmetry:

- MHA native `PagedAttentionImpl` is mostly control flow and kernel dispatch, so the vLLM MHA path can be fully independent.
- MLA native `MLAAttention` owns important model weights and post-load transformation logic, so the vLLM MLA path subclasses it to reuse state setup while overriding the runtime forward path.

The isolation goal is still preserved: native `MLAAttention` is not patched for vLLM, and vLLM-specific MLA execution lives in `AttentionForVllmMLA`.
...
They no longer inherit method-container classes like `vllmAiterAttentionBackendMethods`.
...
The metadata builders are also explicit classes:
...
They no longer use metadata builder decorators or method-container inheritance.

Their `__init__`, `build`, and helper methods are real member methods.
...
```text
atom/model_ops/
  base_attention.py            # frontend Attention dispatcher and native custom op entry
  paged_attention.py           # ATOM native Attention layer
  attention_mha.py             # ATOM native MHA implementation
  attention_mla.py             # ATOM native MLA implementation
  attentions/
    aiter_attention.py         # ATOM native MHA backend and metadata builder
    aiter_mla.py               # ATOM native MLA backend and metadata builder

atom/plugin/vllm/
  attention/
    layer.py                   # AttentionForVllm factory; dispatches MHA/MLA/Sparse MLA
    layer_mha.py               # AttentionForVllmMHA layer and MHA forward implementation
    layer_mla.py               # AttentionForVllmMLA / SparseMLA layer and MLA forward implementation
    layer_common.py            # shared helpers for vLLM layer registration and scale setup
    backend.py                 # vLLM MHA/MLA backend classes and dense metadata builders
    metadata.py                # vLLM attention metadata dataclasses and shared metadata helpers
    ops.py                     # atom_vllm custom ops and graph split markers
    mla_impl.py                # dense MLA helper functions used by layer_mla.py
    mla_sparse_impl.py         # sparse MLA helper functions, indexer op, and indexer adapters
  attention_backend/
    mla_sparse.py              # vLLM sparse MLA backend and sparse/indexer metadata builders
    gdn_attn.py                # vLLM GDN backend integration
    attention_gdn.py           # vLLM GDN attention implementation
  moe.py                       # vLLM-only MoE naming adaptation

atom/plugin/sglang/
  attention.py                 # AttentionForSGLang frontend wrapper
  attention_backend/
    radix_attention.py         # SGLang RadixAttention wrapper
    sgl_attn_backend.py        # SGLang attention backend integration
    sgl_attention_mla.py       # SGLang MLA attention integration
```
